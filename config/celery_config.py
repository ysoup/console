...
if load_dict.__contains__('celery'):
    CELERY_IMPORTS = tuple([x for x in load_dict["celery"]["celery_imports"]])
    CELERY_QUEUES = tuple([Queue(x["queue"], exchange=Exchange(x["exchange"], routing_key=x["routing_key"]))
                           for x in load_dict["celery"]["celery_queues"]])

    CELERYBEAT_SCHEDULE = {x["schedule_name"]: {"task": x["task"], "schedule": x["schedule"],
                                                "options": {"queue": x["queue"], "routing_key": x["routing_key"]}}
                           for x in load_dict["celery"]["celerybeat_schedule"]}
